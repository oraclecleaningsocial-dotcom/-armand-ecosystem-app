...
8. **Apple Health integration is fully mocked.** `connectAppleHealth()`/`syncHealthData()` simulate a HealthKit connection and canned metrics. There are no code comments anywhere in `main.jsx` (zero `//`/`/* */`) — the "this is mock; connect HealthKit/`react-native-health` in React Native" explanation only exists as Italian strings rendered in the `Fitness` screen and the `connectAppleHealth()` return message, not as developer comments. Don't implement real HealthKit calls without being asked — this is a web prototype.
